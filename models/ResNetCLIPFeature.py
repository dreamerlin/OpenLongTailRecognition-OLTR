...
from layers import ModulatedAttLayer
...
class CVLP(nn.Module):
    def __init__(self,
                 ...
                 num_segments=8):
        super().__init__()
        assert isinstance(vision_layers, (tuple, list))
        ...
        if self.use_fc:
            print('Using fc.')
            self.fc_add = nn.Linear(1024, 512)
        if self.use_dropout:
            print('Using dropout.')
            self.dropout = nn.Dropout(p=dropout)
        if self.use_modulatedatt:
            print('Using self attention.')
            self.modulatedatt = ModulatedAttLayer(in_channels=1024)
    ...
    def forward(self, x, train=True):
        ...
        x = x.reshape((-1, self.num_segments, 3) + x.shape[-2:])
        ...
        x = x.reshape(num_batches, self.num_segments, -1).mean(dim=1, keepdim=False)
